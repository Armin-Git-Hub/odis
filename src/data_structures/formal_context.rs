...
pub struct FormalContext<T> {
    ...
    atomic_object_derivations: Vec<BTreeSet<usize>>,
    atomic_attribute_derivations: Vec<BTreeSet<usize>>,
}

impl<T> FormalContext<T> {
    fn construct(objects: Vec<T>, attributes: Vec<T>, incidence: HashSet<(usize, usize)>) -> Self {
        let mut atomic_object_derivations = vec![BTreeSet::<usize>::new(); objects.len()];
        let mut atomic_attribute_derivations = vec![BTreeSet::<usize>::new(); attributes.len()];
        ...
        FormalContext {
            ...
            incidence: incidence,
            atomic_object_derivations: atomic_object_derivations,
            atomic_attribute_derivations: atomic_attribute_derivations,
        }
    }
    ...
    pub fn index_attribute_derivation(&self, attributes: &BTreeSet<usize>) -> BTreeSet<usize> {
        match attributes.len() {
            0 => (0..self.objects.len()).collect(),
            1 => self.atomic_attribute_derivations[*attributes.first().unwrap()].clone(),
            _ => {
                let mut current_value = 0;
                std::iter::from_fn(|| 'outer: loop {
                    for &x in attributes {
                        if let Some(&next) = self.atomic_attribute_derivations[x]
                            .range(current_value..)
                            .next()
                        {
    ...
    pub fn index_object_derivation(&self, objects: &BTreeSet<usize>) -> BTreeSet<usize> {
        match objects.len() {
            0 => (0..self.attributes.len()).collect(),
            1 => self.atomic_object_derivations[*objects.first().unwrap()].clone(),
            _ => {
                let mut current_value: usize = 0;
                std::iter::from_fn(|| 'outer: loop {
                    for &x in objects {
                        if let Some(&next) = self.atomic_object_derivations[x]
                            .range(current_value..)
                            .next()
                        {
                            if next != current_value {
    ...
    pub fn index_attribute_hull(&self, attributes: &BTreeSet<usize>) -> BTreeSet<usize> {
    ...
    pub fn index_object_hull(&self, objects: &BTreeSet<usize>) -> BTreeSet<usize> {
